...
typedef struct set{ // set 구조체 
	int file_num; // 파일 수 
	unsigned char hashnum[16]; // 해시값 
	int size; // 파일 사이즈 
	struct set* ptr; // 다음 set 포인터 
	setnode* head;	 //안에 있는 setnode 맨 앞과 뒤 포인터 
	setnode* tail;
}set;

int check_ = 0;
char queue[QUEUE_MAX][4097]; // 큐 
int file_max;  // 허용하는 파일 최대 최소 사이즈 
int file_min;
int runcnt;
struct tm *time_info; // 시간 저장 구조체 
set* set_head; // set 맨 앞과 끝 포인터 
set* set_tail;
int front; 
int rear;
unsigned char data[4096]; // 해시 데이터 
char file_extension[10]; // 확장자 저장소 
char buf[4096]; // 버퍼 
setnode *sn;
setnode *snn;
void PrintTime(struct stat *st){ // 시간 출력 함수 
	struct tm *t;
...
int Full(void){ // queue 다 찼는지 확인 함수 
...
void push(char *name){ // queue push 함수 
	if(Full()){
...
void MakeBound(int a,char* num){ // 파일 범위 정하는 함수 
	if(num[0]=='~'){
...
setnode *s; 
setnode* MakeSetnode(char *n){ // 주어진 이름으 setnode 구조체 만들어서 리턴하는 함수 
	s = (setnode *)calloc(1,sizeof(setnode));
	char *nam = (char *)malloc(strlen(n)+1); // 동적할당 
...
void PrintAll(){ // 링크드리스트 전체 출력 함수 ( 에러 체킹용)
	set* st = set_head;
	printf("\n");
	while(st!=NULL){
		setnode *stn = st->head;
...
void RemoveList(){ // 링크드리스트 전체 삭제 함수 
	set* setptr = set_head;
	set* setptr2 =set_head;

	while(setptr!=NULL){ // set 포인터 파일 제일 끝 가기 전까지 
		setptr2 = setptr->ptr;
		setnode* setn=setptr->head;
		setnode* setn2=setptr->head;
		while(setn!=NULL){ //안에 있는 setnode 링크드리스트부터 비워준다.
			setn2 = setn->ptr;
			free(setn);
			setn=setn2;
		}
...
char nametmp[4095];
void fmd5(char *File_ex, char* Min, char* Max , char* Target){ // fmd5 함수 
	file_init();
	...
	push(nametmp);
	DIR *dir=NULL; // dir 포인터 생성 
	struct dirent *file = NULL;
	char tmpname[4096];
	struct stat st;
	FILE* fp;
	unsigned char md5tmp[16]; // 해시값 임시저장 
	struct timeval start; // 시간 구하기 
	struct timeval end;
	...
	while(front!=rear){ // 디렉토리면 탐색, 큐 추가하고, 파일이면 해쉬 비교후 리스트 추가한다.
		...
		while((file = readdir(dir))!=NULL){ // 디렉토리 탐색
			...
			if(stat(tmpname,&st)==-1){ // stat으로 tmpname의 구조체 포인트 
			...
			if(S_ISDIR(st.st_mode)){ // 디렉토리라면
				...
				if((fp =fopen(tmpname, "rb"))==NULL){ // 비트로 파일 읽기 
				...
				MD5_Init(&ctx);
				int bytes;
				while((bytes = fread(data, 1, 4096,fp))!=0) // data 를 업데이트해줌 
					MD5_Update(&ctx, data, bytes);
				MD5_Final(md5tmp, &ctx); // md5tmp에 해시값 생성 
				//@@@@@@@@@@@@@
				/*
				...
				*/
				...
				while(setptr!=NULL){ // set 링스크리스트 탐색 
					if(setptr->size ==st.st_size){ // 파일 사이즈 같음 
						int tx;
						tx=strcmp(setptr->hashnum,md5tmp);

						if((tx=strncmp(setptr->hashnum,md5tmp,16))==0){ // 해시값도 같음
							setptr->file_num +=1;
	...
	while(1){
		...
		while(stmp!=NULL){ // set 순환 , 출력
			if(stmp->file_num>1){ // 같은 해시 가진게 2개 이상이라면 
				printf("---- Identical files #%d (%d bytes - ",++setcnt,stmp->size); // 해시 출력 
				for(int i=0;i<16;i++){
				...
				while(sn!=NULL){ // setnode 순환
					if(realpath(sn->name,buf)==NULL){ // 절대경로 변환 
		...
		while(1){
			if(setcnt!=0){ // 지우는 옵션구현
				...
				for(int i=0;i<10;i++){ // fgets 맨 뒤 개행문자 변환 
					if(option[i]=='\n'){
						option[i]='\0';
					}
				}
				char *opptr = strtok(option, " "); // 띄어쓰기 기준 토큰화 
				while(opptr!=NULL){
					ptrset[cn++]=opptr;
					opptr = strtok(NULL," ");
				}

				if(strcmp(ptrset[0],"exit")==0){ //exit 구현 
					...
					if(selectnum>setcnt){
						...
						if(ptrset[2]==NULL){ // 2번쨰 옵션 없음 
							...
							for(int s=1;s<selectnum;s++){ // 주어진 수에 따라 이동 
								stmp = stmp->ptr;
							}
							...
							sn = stmp->head;
							snn = stmp->head;
							for(int s=1;s<nodenum;s++){ // setnode도 주어진 수 맞게 이동 
								if(s!=1){
									snn=snn->ptr;
								}
								sn = sn->ptr;
						...
						snn = stmp->head;
						char yn[3];
						while(sn!=NULL){ // setnode 링크드 리스트 끝까지 확인 
							realpath(sn->name,buf);
							...
							if(snn!=sn){ // 맨 처음만 snn 포인터 하나만 이동 
								snn=snn->ptr;
							}
							sn = sn->ptr;
...
int main(int argc, char* argv[]){
	fmd5(argv[0],argv[1],argv[2],argv[3]);
